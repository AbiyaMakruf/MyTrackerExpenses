...
<section class="glass-card space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('records.edit', $transaction) }}" class="btn-primary inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold transition-all duration-200 ease-out hover:scale-[1.02] hover:brightness-110">
                {{ __('Edit transaction') }}
            </a>
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <div class="rounded-2xl border border-[#D2F9E7] bg-white/80 p-5">
            <dl class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <dd class="text-sm text-slate-600">{{ $transaction->note ?? '—' }}</dd>
                </div>
            </dl>
        </div>
        <div class="rounded-2xl border border-[#D2F9E7] bg-white/80 p-5">
                ...
                <div>
                    ...
                    <dd>
                        @if ($transaction->attachment_path)
                            <div class="mt-2">
                                @php
                                    $url = Str::startsWith($transaction->attachment_path, 'http') 
                                        ? $transaction->attachment_path 
                                        : Storage::disk('public')->url($transaction->attachment_path);
                                    $isImage = Str::endsWith(strtolower($transaction->attachment_path), ['.jpg', '.jpeg', '.png', '.gif', '.webp']);
                                @endphp
                                
                                @if ($isImage)
                                    <img src="{{ $url }}" class="mb-2 max-h-64 rounded-lg object-contain border border-[#D2F9E7]" />
                                @endif
                                
                                <a href="{{ $url }}" class="text-[#08745C] underline hover:text-[#095C4A]" target="_blank">
